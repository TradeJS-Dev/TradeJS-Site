...
export function StrategyAuthoring() {
  ...
  return (
    <section id="advantages" className="relative overflow-hidden border-b border-border bg-background pb-12 pt-24 text-foreground lg:pb-16 lg:pt-28">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 lg:px-8">
        <AnimateOnScroll>
          <div className="mb-12 grid gap-8 md:grid-cols-[1.1fr_0.9fr] md:items-end lg:mb-16">
            <div>
              <div className="mb-5 font-mono text-[10px] tracking-[0.22em] text-primary">02 / THE SYSTEM</div>
              ...
              <article className="grid gap-10 py-10 lg:grid-cols-12 lg:items-center lg:gap-14 lg:py-20">
                <div className={i % 2 === 1 ? 'lg:order-2 lg:col-span-4 lg:col-start-9' : 'lg:col-span-4'}>
                  <div className="mb-8 flex items-center gap-4">
                    <span className="font-mono text-[10px] tracking-[0.2em] text-primary">0{i + 1}</span>
                    <span className="h-px w-10 bg-primary/35" />
                    <span className="font-mono text-[10px] tracking-[0.2em] text-muted-foreground">{labels[i]}</span>
                  </div>
                  <h3 className="text-3xl font-semibold leading-tight tracking-[-0.03em] sm:text-4xl">{item.title}</h3>
                  <p className="mt-5 text-base leading-relaxed text-muted-foreground">{item.description}</p>
                  <div className="mt-8 flex flex-wrap gap-x-5 gap-y-3">
                    {item.links.map((link) => (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 border-b border-primary/30 pb-1 text-xs font-semibold text-primary transition-colors hover:border-primary hover:text-accent-hover"
                      >
                        {link.label}
